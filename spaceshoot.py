...
FPS = 60
pygame.display.set_caption("SPACE MAN X")
...
#background and scale it to the window size
...
def main():
    ...
    while run:
        ...
        if len(enemies) == 0:
            level += 1
            wave_length += 2 * level
            lives = 5 # after each level reset player lives and healthbar
            player.health = 100
        ...
        if keys[pygame.K_a] or keys[pygame.K_LEFT] and player.x - player_vel > 0: #move left
            player.x -= player_vel
        if keys[pygame.K_d] or keys[pygame.K_RIGHT] and player.x + player_vel + player.get_width() < WIDTH: #move right
            player.x += player_vel
        if keys[pygame.K_w] or keys[pygame.K_UP] and player.y - player_vel > 0: #move up
            player.y -= player_vel
        if keys[pygame.K_s] or keys[pygame.K_DOWN] and player.y + player_vel + player.get_height() + 15 < HEIGHT: #move down
            player.y += player_vel
